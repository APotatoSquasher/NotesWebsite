...
def login():
    if request.method == 'POST':
        ...
        user = User.query.filter_by(email=email).first() # return emails with email
        if user:
...
def sign_up():
    if request.method == 'POST':
        ...
        if user:
            ...
            db.session.commit()
            login_user(user,remember=True)
            flash("Congrats! You've made an account!",category='success')
            return redirect(url_for('views.home'))
    return render_template("sign_up.html",user=current_user)
#at sign up page html chapter